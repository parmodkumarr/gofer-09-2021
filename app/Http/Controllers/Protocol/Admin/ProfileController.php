...
class ProfileController extends Controller
{
    public function adminProfile(Request $request)
    {
    	 ...
    	 $admin= DB::connection('mysql_sec')->table('admin')
    	  ...
    	  $logo = DB::connection('mysql_sec')->table('tbl_web_setting')
           ...
           return view('protocol.admin.profile.profile',compact("admin_email","admin",'logo',"title"));
    ...
    public function adminUpdateProfile(Request $request)
    {
        ...
        $getImage = DB::connection('mysql_sec')->table('admin')
                        ->where('id', $admin_id)
                        ->first();
        ...
        $adminChangeProfile = DB::connection('mysql_sec')->table('admin')
                                ->where('id', $admin_id)
        ...
        $title = "Change Password";
    	 ...
    	 $admin= DB::connection('mysql_sec')->table('admin')
    	  ...
    	  $logo = DB::connection('mysql_sec')->table('tbl_web_setting')
                ->where('set_id', '1')
                ->first();	
           return view('protocol.admin.profile.change_pass',compact("admin_email","admin","logo","title"));
   ...
   public function adminChangePassword(Request $request)
    {
        ...
        $getAdmin = DB::connection('mysql_sec')->table('admin')
                    ->where('id', $admin_id)
                    ->first();
        ...
        if(Hash::check($current_pass,$getAdmin->admin_pass))
            ...
            $adminChangePassword = DB::connection('mysql_sec')->table('admin')
                                    ->where('id', $admin_id)

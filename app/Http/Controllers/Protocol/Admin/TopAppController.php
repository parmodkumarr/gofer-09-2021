...
class TopAppController extends Controller
{
    public function adminTopCat(Request $request)
    {
    	...
    	return view('protocol.admin.topcat.index',compact("category", "title","admin","logo"));
    }
    ...
    public function adminAddTopCat(Request $request)
    {
        $title = "Add Home Category";
    	 ...
    	 $admin= DB::connection('mysql_sec')->table('admin')
    	  ...
    	  $logo = DB::connection('mysql_sec')->table('tbl_web_setting')
                ->where('set_id', '1')
                ->first();   
        $topapp = DB::connection('mysql_sec')->table('tbl_top_cat')
                    ->get();
        if(count($topapp)>0){
    	...
    	return view('protocol.admin.topcat.add', compact("app", "allPosition", "title","admin","logo"));
    }
    ...
    public function adminAddNewTopCat(Request $request)
    {
    	...
    	$insertTopApp = DB::connection('mysql_sec')->table('tbl_top_cat')
    	                   ->insert($app + [
    ...
    public function adminEditTopCat(Request $request)
    {
        ...
        $getApp = DB::connection('mysql_sec')->table('tbl_top_cat')
                    ->where('id', $id)
                    ->first();
        ...
        $app = DB::connection('mysql_sec')->table('categories')
    	 ...
    	 $admin= DB::connection('mysql_sec')->table('admin')
    	  ...
    	  $logo = DB::connection('mysql_sec')->table('tbl_web_setting')
                ->where('set_id', '1')
                ->first();  
        return view('protocol.admin.topcat.edit', compact("app", "allPosition", "getCatId", "id", "getCatRank", "title","admin","logo"));
    }
    ...
    public function adminUpdateTopCat(Request $request)
    {
        ...
        $updateTopApp = DB::connection('mysql_sec')->table('tbl_top_cat')
                           ->where('id', $id)
                           ->update($app + [
                                'updated_at'=>$updated_at
                            ]);
    ...
    public function adminTopCatDelete(Request $request)
    {
        ...
        $getPosition = DB::connection('mysql_sec')->table('tbl_top_cat')
                        ->where('id', $id)
                        ->first();
                        
        $getRank = $getPosition->cat_rank;

        $deleteTopApp = DB::connection('mysql_sec')->table('tbl_top_cat')
                            ->where('id', $id)
                            ->delete();

        if($deleteTopApp){
            
            $selectApps = DB::connection('mysql_sec')->table('tbl_top_cat')
            ...
            foreach($selectApps as $selectApp){
                ...
                $updateRank = DB::connection('mysql_sec')->table('tbl_top_cat')

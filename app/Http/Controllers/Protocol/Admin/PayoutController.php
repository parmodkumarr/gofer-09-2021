...
class PayoutController extends Controller
{
    ...
    public function pay_req(Request $request)
    {
         ...
         $total_earnings=DB::connection('mysql_sec')->table('payout_requests')
                           ...
                           ->select('store.store_id','store.store_name', 'store.phone_number','store.address','store.email','store_earning.paid','payout_requests.payout_amt', 'payout_requests.complete','payout_requests.req_id','store_bank.ac_no', 'store_bank.ifsc','store_bank.holder_name','store_bank.bank_name', 'store_bank.upi', DB::connection('mysql_sec')->raw('SUM(orders.total_price)-SUM(orders.total_price)*(store.admin_share)/100 as sumprice'))
    	...
    	return view('protocol.admin.store.payoutRequest', compact('title',"admin", "logo","total_earnings"));
    }
     ...
     public function store_pay(Request $request)
    {
        ...
        $st = DB::connection('mysql_sec')->table('payout_requests')
            ->where('req_id',$req_id) 
            ->first();
        ...
        $logo = DB::connection('mysql_sec')->table('tbl_web_setting')
                ...
                ->first();
        $amt = $request->amt;
        $check = DB::connection('mysql_sec')->table('store_earning')
                ->where('store_id',$store_id)
                ->first();
        $check2 = DB::connection('mysql_sec')->table('store')
                ->where('store_id',$store_id)
                ->first();        
        ...
        $update = DB::connection('mysql_sec')->table('store_earning')
                ->where('store_id',$store_id)
         ...
         $update = DB::connection('mysql_sec')->table('store_earning')
                ->insert(['store_id'=>$store_id,'paid'=>$amt]);   
                 ...
                 DB::connection('mysql_sec')->table('payout_requests')
                 ->where('req_id', $req_id)
                ->update(['complete'=>1]);
        }
        if($update){

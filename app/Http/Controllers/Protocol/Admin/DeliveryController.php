...
class DeliveryController extends Controller
{
    ...
    {
        $d_boy = DB::connection('mysql_sec')->table('delivery_boy')
            ->paginate(10);
         if($request->ajax()){
           return view('protocol.admin.d_boy.pagination_data',compact('d_boy'))->render();
        }
    	...
    	return view('protocol.admin.d_boy.list', compact('title',"admin", "logo","d_boy"));
    }
     ...
     public function AddD_boy(Request $request)
    {
        ...
        $d_boy = DB::connection('mysql_sec')->table('delivery_boy')->get();
        ...
        return view('protocol.admin.d_boy.add',compact("d_boy", "admin_email","logo", "admin","title", 'city','map','mapset','mapbox'));
     }
     ...
     public function AddNewD_boy(Request $request)
    {
        ...
        $chkboyrphon = DB::connection('mysql_sec')->table('delivery_boy')
        ...
        $insert = DB::connection('mysql_sec')->table('delivery_boy')
                            ->insert([
                                'boy_name'=>$boy_name,
                                'boy_phone'=>$boy_phone,
                                'boy_city'=>$city,
    ...
    public function EditD_boy(Request $request)
    {
        ...
        $mapbox = DB::connection('mysql_sec')->table('mapbox')
                ->first();
        return view('protocol.admin.d_boy.edit',compact("d_boy","admin_email","admin","logo","title","city","map",'mapset','mapbox'));
    }
    ...
    {// echo "<pre>";print_r($request->all());die;
     ...
     //    }
        ...
        $updated = DB::connection('mysql_sec')->table('delivery_boy')
                   ->where('dboy_id', $dboy_id)
                    ->update([
                        'boy_name'=>$boy_name,
                        'boy_phone'=>$boy_phone,
 ...
 public function DeleteD_boy(Request $request)
    {
    	...
    	$delete=DB::connection('mysql_sec')->table('delivery_boy')

  ...
  public function payout_req(Request $request)
    {
    	...
    	$store_email=Session::get('bamaStore');
         ...
         $ll = DB::connection('mysql_sec')->table('payout_requests')
            ->where('store_id',$store->store_id)
            ->orderBy('req_id', 'desc')
            ->first();
         $dd = DB::connection('mysql_sec')->table('payout_req_valid')  
             ->first();
	  ...
	  }else
	  {
         ...
         $bank = DB::connection('mysql_sec')->table('store_bank')
                ->where('store_id',$store->store_id)
                ->first();
        ...
        $currency = DB::connection('mysql_sec')->table('currency')
                  ->first();
         return view('protocol.store.payout_req',compact("store_email", "store",'title', 'logo', 'sumprice', 'paid', 'bank', 'total_earnings', 'currency', 'current', 'end', 'last_date', 'min_amt','days','ll'));
    } 
    public function req_sent(Request $request)
    {
        ...
        $check = DB::connection('mysql_sec')->table('store_bank')
              ->where('store_id',$store->store_id)
               ->first();
      ...
      if($check){
        ...
        $update = DB::connection('mysql_sec')->table('store_bank')
                ->where('store_id',$store->store_id)
      ...
      $up1 = DB::connection('mysql_sec')->table('payout_requests')
       ->where('complete', 0)
       ->where('store_id', $store->store_id)
       ->update(['complete'=> 2]);
       
       
      $up2 = DB::connection('mysql_sec')->table('payout_requests') 

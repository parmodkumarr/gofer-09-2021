...
class HomeLayoutController extends Controller{
    ...
    public function homeLayoutFormView(Request $request){
        ...
        if($sectionDetail->section_type == 1){
            ...
            return view('protocol.store.home_design.home_layout.home_layout_for_category', compact('title','admin','logo','admin_email','sectionTypes','section_id','sectionDetail','store','layout_id','sectionTypeDetail','appLayoutTypes','ifAvailableLayout'));

        }elseif($sectionDetail->section_type == 2){

            return view('protocol.store.home_design.home_layout.home_layout_for_sub_category', compact('title','admin','logo','admin_email','sectionTypes','section_id','sectionDetail','store','layout_id','appLayoutTypes','ifAvailableLayout'));

        }elseif($sectionDetail->section_type == 3){

            return view('protocol.store.home_design.home_layout.home_layout_for_product', compact('title','admin','logo','admin_email','sectionTypes','section_id','sectionDetail','store','layout_id','appLayoutTypes','ifAvailableLayout'));
        }
    }

...
class AddressController extends Controller
{
     public function address(Request $request)
    {
            ...
            $unselect= DB::connection('mysql_sec')->table('address')
    	    ...
    	    $insertaddress = DB::connection('mysql_sec')->table('address')
    						->insert([
    							'user_id'=>$user_id,
    ...
    $society= DB::connection('mysql_sec')->table('society')
   ...
   $store = DB::connection('mysql_sec')->table('store')
    ...
    $address = DB::connection('mysql_sec')->table('address')
         ...
         ->select('address.*',DB::connection('mysql_sec')->raw("6371 * acos(cos(radians(".$store->lat . ")) 
       ...
       if(count($address)>0){
		   foreach($address as $addresses)
		   {
		    ...
		    $check = DB::connection('mysql_sec')->table('address')
             ->WhereIn('address_id',$address_id)
		     ->where('select_status',1)
		     ->get();
    if(count($check)==0){
		   $selected =   DB::connection('mysql_sec')->table('address')
         ->where('user_id',$user_id)
         ->where('select_status',1)
	     ->update(['select_status'=>0]);
	}
...
public function select_address(Request $request)
    ...
    $user = DB::connection('mysql_sec')->table('address')
         ->where('address_id',$address_id)
         ->first();
    $checkuser = $user->user_id;  
    $select1 = DB::connection('mysql_sec')->table('address')
         ->where('user_id',$checkuser)
         ->update(['select_status'=> 0]);
    $select = DB::connection('mysql_sec')->table('address')
         ->where('address_id',$address_id)
...
public function rem_user_address(Request $request)
    {
    $address_id = $request->address_id;
    $checkcart = DB::connection('mysql_sec')->table('orders')
               ->where('address_id', $address_id)
               ->get();
    if(count($checkcart)==0) {
        $deladdress= DB::connection('mysql_sec')->table('address')
         ->where('address_id',$address_id)
         ->delete();

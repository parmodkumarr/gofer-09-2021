...
class RegisterController extends Controller
{
     ...
     public function register_user(Request $request)
  {
    ...
    $logo = DB::connection('mysql_sec')->table('tbl_web_setting')
  	...
  	return view('protocol.web.auth.register', compact('logo'));
  }
    ...
    public function usersignup(Request $request)
    {
    	...
    	$checkUser = DB::connection('mysql_sec')->table('users')
	...
	elseif($checkUser && $checkUser->is_verified==0){
	                 $delnot= DB::connection('mysql_sec')->table('notificationby')
    						->where('user_id', $checkUser->user_id)
    				     	->delete();
    						
    	    	$delUser = DB::connection('mysql_sec')->table('users')
    					->where('user_phone', $user_phone)
    					->delete();
    	   ...
    	   $user_image = 'N/A';
    		...
    		$insertUser = DB::connection('mysql_sec')->table('users')
    						->insertGetId([
    							'user_name'=>$user_name,
            	...
            	$Userdetails = DB::connection('mysql_sec')->table('users')
    					->where('user_phone', $user_phone)
    					->first();
    		if($insertUser){
    		     DB::connection('mysql_sec')->table('notificationby')
                ...
                $updateOtp = DB::connection('mysql_sec')->table('users')
                                ->where('user_phone', $user_phone)
                                ->update(['otp_value'=>$otpval]);
    						
	    		return view('protocol.web.auth.otp',compact('user_phone'));
	    	}
	    	else{
    ...
    public function login(Request $request)
    
     {
    	...
    	$checkUserReg = DB::connection('mysql_sec')->table('users')
    					->where('user_phone', $user_phone)
    					->first();
    	...
    	if($checkUser){
    	    ...
    	    if($checkUser->is_verified == 0){
               ...
               $otpmsg = $this->otpmsg($otpval,$user_phone);
                ...
                $checkUser1 = DB::connection('mysql_sec')->table('users')
            					->where('user_phone', $user_phone)
            					->first();                
    		   ...
    		   $updateDeviceId = DB::connection('mysql_sec')->table('users')
    		                        ->where('user_phone', $user_phone)
     ...
     public function profile_edit(Request $request)
    {
    	...
    	$user_phone = $request->user_phone;
    	$user_image = $request->user_image;
    		$uu = DB::connection('mysql_sec')->table('users')
    	    ->where('user_id', $user_id)
    	    ->first();

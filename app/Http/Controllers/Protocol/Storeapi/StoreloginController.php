...
class StoreloginController extends Controller{
    ...
    public function store_login(Request $request){
    	...
    	$checkstore1 = DB::table('store')->where('email', $email)->first();
        
        if($checkstore1){
    	   $checkstore = DB::table('store')->where('email', $email)->where('password', $password)->first();

        	if($checkstore){
        	    if($checkstore->admin_approval==1){
        		   $updateDeviceId = DB::table('store')->where('email', $email)->update(['device_id'=>$device_id]);

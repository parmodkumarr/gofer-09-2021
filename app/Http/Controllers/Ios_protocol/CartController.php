...
namespace App\Http\Controllers\Ios_protocol;
...
class CartController extends Controller
{
   ...
   public function add_to_cart(Request $request)
    {   
      ...
      $price = $p->price;
        ...
        $check = DB::connection('mysql_sec')->table('store_orders')
     ...
     if(!$check){
        ...
        $insert = DB::connection('mysql_sec')->table('store_orders')
          ...
          $del = DB::connection('mysql_sec')->table('store_orders')
            ->where('store_approval',$user_id)
            ->where('varient_id',$varient_id)
            ->where('order_cart_id', "incart")
            ->delete();
  ...
  if($insert){
      $del = DB::connection('mysql_sec')->table('store_orders')
            ...
            ->where('qty', 0)
            ->delete();
      $sum = DB::connection('mysql_sec')->table('store_orders')
            ->where('store_approval',$user_id)
            ->where('order_cart_id', "incart")
            ->select(DB::connection('mysql_sec')->raw('SUM(store_orders.price) as sum'))
            ->first();
            
            
      $checkitems = DB::connection('mysql_sec')->table('store_orders')
            ->where('store_approval',$user_id)
            ->where('order_cart_id', "incart")
            ->get();
  ...
  public function make_an_order(Request $request)
    {   
        ...
        $store_id = $request->store_id;
        $data_array = DB::connection('mysql_sec')->table('store_orders')
              ->where('store_approval', $user_id)
              ->where('order_cart_id',"incart")
              ->get();
        $chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
    ...
    { 
      ...
      $price = $p->price;
        ...
        $order_qty = $h->qty;
        $p = DB::connection('mysql_sec')->table('product_varient')
           ->join('product','product_varient.product_id','=','product.product_id')
           ->where('product_varient.varient_id',$varient_id)
           ->first(); 
  ...
  if($insert){
        $oo = DB::connection('mysql_sec')->table('orders')
           ...
           $ordersuccessed = DB::connection('mysql_sec')->table('orders')
            ...
            $delete = DB::connection('mysql_sec')->table('store_orders')
                           ->where('store_approval',$user_id)
                           ->where('order_cart_id', 'incart')
                           ->delete();
   ...
   public function show_cart(Request $request)
    { 
        $user_id= $request->user_id;
        $cart_items = DB::connection('mysql_sec')->table('store_orders')
                    ->where('store_approval',$user_id)
                    ->where('order_cart_id', 'incart')
                    ->get();

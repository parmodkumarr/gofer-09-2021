...
class IostimeslotController extends Controller
{
 public function timeslot(Request $request)
    {
    $current_time = Carbon::Now();
    $date = date('Y-m-d');
    $time_slot = DB::connection('mysql_sec')->table('time_slot')
                    ->first();
...
elseif(strtotime($date)>strtotime($selected_date)){
                ...
                $message = array('status'=>'0', 'message'=>"You can't select the back date", 'data'=>[]);
            return $message; 
}
else{
    while ($start_time <= $end_time) // loop between time
    ...
    if(count($new_array_of_time1)>0){
            ...
            {
                $message = array('status'=>'0', 'message'=>'Oops No time slot present', 'data'=>[]);
            return $message;
            }

...
class DriverloginController extends Controller
{
 public function driver_login(Request $request)
    
     {
    	...
    	$checkdriver1 = DB::connection('mysql_sec')->table('delivery_boy')
    					->where('boy_phone', $phone)
    					->first();
    if($checkdriver1){					
    	$checkdriver = DB::connection('mysql_sec')->table('delivery_boy')
    					->where('boy_phone', $phone)
    					->where('password', $password)
    					->first();

    	if($checkdriver){
    		   $updateDeviceId = DB::connection('mysql_sec')->table('delivery_boy')
    		                       ->where('boy_phone', $phone)
    ...
    {   
        $boy_id = $request->dboy_id;
         $diver=  DB::connection('mysql_sec')->table('delivery_boy')

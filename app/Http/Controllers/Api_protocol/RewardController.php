...
class RewardController extends Controller
{
   public function redeem(Request $request)
    {  
        ...
        $rewards = DB::connection('mysql_sec')->table('users')
    ...
    {  
        ...
        $check = DB::connection('mysql_sec')->table('orders')
               ->where('cart_id',$cart_id)
               ->first();
        $p=$check->total_price;
        $currency = DB::connection('mysql_sec')->table('currency')
                  ->first();
        $cc = DB::connection('mysql_sec')->table('reward_points')
          ...
          $cc2 = DB::connection('mysql_sec')->table('reward_points')

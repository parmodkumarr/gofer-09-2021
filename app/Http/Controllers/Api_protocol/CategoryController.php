...
class CategoryController extends Controller
{
 public function cate(Request $request)
     ...
     $cat = DB::connection('mysql_sec')->table('categories')
        ...
        if(count($cat)>0){
            ...
            foreach ($cat as $cats) {
                ...
                $apps = array($app);
                $app = DB::connection('mysql_sec')->table('categories')
                        ->whereIn('parent', $apps)
                        ->where('level',1)
                        ->get();
                ...
                foreach ($app as $appss) {
                    ...
                    $app1 = DB::connection('mysql_sec')->table('categories')
                            ->whereIn('parent', $c)
                            ->where('level',2)
                            ->get();
                if(count($app1)>0){        
  ...
  public function cat_product(Request $request)
    {
$nearbystore = DB::connection('mysql_sec')->table('store_products')->update(['stock'=>100]);
            ...
            foreach ($cat as $cats) {
                ...
                if(count($app1)>0){        
                    ...
                    foreach ($app1 as $apps1) {
                        ...
                        $prod = DB::connection('mysql_sec')->table('product')
                                ->whereIn('cat_id', $catt)
                                ->get();
                     ...
                     $k++;   
                       ...
                       $pr = DB::connection('mysql_sec')->table('product')
                        ->whereIn('cat_id', $c)
                        ->get();    
                        $res[$j]->product = $pr;
                        $j++; 
                ...
                $app = DB::connection('mysql_sec')->table('product')
                        ->whereIn('cat_id', $apps)
                        ->get();    
...
if($nearbystore){                  
    if($nearbystore->del_range >= $nearbystore->distance)  {                
        $varient= DB::connection('mysql_sec')->table('store_products')
                 ->join ('product_varient', 'store_products.varient_id', '=', 'product_varient.varient_id')
                 ->Leftjoin('deal_product','product_varient.varient_id','=','deal_product.varient_id')
                         ->select('store_products.store_id','store_products.stock','product_varient.varient_id', 'product_varient.description', 'store_products.price', 'store_products.mrp', 'product_varient.varient_image','product_varient.unit','product_varient.quantity','deal_product.deal_price', 'deal_product.valid_from', 'deal_product.valid_to')
    ...
    if($nearbystore){  
       if($nearbystore->del_range >= $nearbystore->distance)  {        
        $deal_p= DB::connection('mysql_sec')->table('deal_product')
...
if($nearbystore){  
    if($nearbystore->del_range >= $nearbystore->distance)  {                 
      $topsix = DB::connection('mysql_sec')->table('store_products')
                 ->join ('product_varient', 'store_products.varient_id', '=', 'product_varient.varient_id')
                  ->join ('product', 'product_varient.product_id', '=', 'product.product_id')
                  ...
                  ->select('categories.title', 'categories.image', 'categories.description','categories.cat_id',DB::connection('mysql_sec')->raw('count(store_orders.varient_id) as count'))
...
if($nearbystore){  
    if($nearbystore->del_range >= $nearbystore->distance)  {      
     $category = DB::connection('mysql_sec')->table('tbl_top_cat')
   ...
   public function homepage(Request $request)
    {
       ...
       $lat = $request->lat;
       $lng = $request->lng;
               $banner = DB::connection('mysql_sec')->table('banner')
                ->get();
       $banner2 = DB::connection('mysql_sec')->table('secondary_banner')
                ->get();
       $nearbystore = DB::connection('mysql_sec')->table('store')
                    ->select('del_range','store_id',DB::connection('mysql_sec')->raw("6371 * acos(cos(radians(".$lat . ")) 
                    * cos(radians(store.lat)) 
